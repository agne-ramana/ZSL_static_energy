...
two candidate interfaces next to the input (reference):
  ...
  nooverlap_01  - first-layer Hf as far as possible from the Sr in that plane
...
Scores per shift (lateral = in-plane distance, periodic):
  ...
  rMmin   smallest lateral distance between a first-layer Hf and a plane Sr.
...
First-layer Hf = film Hf within HF_LAYER_DEPTH of the lowest film Hf; film-bottom
...
CLASH_R = 1.5          # A, Hf counts as "over a Sr" below this lateral distance
...
COLORS = {"Sr": "#3fa34d", "Hf": "#8a5fd6", "O": "#d64550"}
...
def main():
    ...
    v1, v2 = surface_vectors(pos[plane_Sr, :2], cell2)
    ...
    print(f"input              : {src}")
    print(f"SrO plane          : z = {z_top:.3f} A, {plane_O.sum()} O, {plane_Sr.sum()} Sr")
    ...
    for k, t in enumerate(shifts):
        ...
        r_m = nearest_lateral(xy_hf, pos[plane_Sr, :2], cell2)       # Hf -> nearest plane Sr
    ...
    # --- tables ---
    cols = ["u", "w", "shift_x", "shift_y", "align", "n_on_top", "n_clash", "min_r_Hf_Sr", "O_O_stack", "d_min"]
    ...
    lines = [f"Registry scan of {src}",
             f"SrO plane: {plane_O.sum()} O, {plane_Sr.sum()} Sr; {hf1.sum()} first-layer Hf; "
             f"{o_bot.sum()} film-bottom O; grid {g} x {g}",
             f"align: sigma {SIGMA_O} A, onTop < {ON_TOP_R} A;  rMmin = min lateral Hf-Sr, clash < {CLASH_R} A;  "
    ...
    for ax, vals, title, cmap in [(axes[0, 0], align, "align (Hf on plane O), maximise", "viridis"),
                                  (axes[0, 1], r_m_min, "min lateral Hf-Sr distance (A), maximise", "magma")]:
    ...
    rm_label = "min lateral Hf-Sr distance (A)"
    clash_label = f"Hf clash count (Hf within {CLASH_R} A of a plane Sr)"
    for ax, x, y, xlabel, ylabel, title in [
            ...
            (axes[1, 1], r_m_min, n_clash, rm_label, clash_label, "min Hf-Sr distance vs Hf clashes")]:
    ...
    for ax, (name, i) in zip(axes, cands):
        ...
        ax.scatter(*wrap_xy(pos[plane_Sr, :2]).T, s=70, c=COLORS["Sr"], lw=0, label="Sr (plane)")
        ...
        ax.set_title(f"{name}: shift ({t[0]:+.2f}, {t[1]:+.2f}) A\n"
                     ...
                     f"min Hf-Sr {r_m_min[i]:.2f} A (clash {n_clash[i]})\n"
                     f"O-O stacked {oo_stack[i]}, d_min {d_min[i]:.2f} A", fontsize=8)
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="lower center", ncol=4, fontsize=9)
    fig.suptitle("Candidate registries, top view (filled = STO plane, hollow = film)", fontsize=11)
